# Faça um Programa que pergunte quanto você ganha por hora e o número de horas trabalhadas no mês. 
# Calcule e mostre o total do seu salário no referido mês, sabendo-se que são descontados 11% para 
# o Imposto de Renda, 8% para o INSS e 5% para o sindicato, faça um programa que nos dê:
# f. salário bruto.
# g. quanto pagou ao INSS.
# h. quanto pagou ao sindicato.
# i. o salário líquido.
# j. calcule os descontos e o salário líquido, conforme a tabela abaixo:+ Salário Bruto : R$ 
# - IR (11%) : R$ 
# - INSS (8%) : R$ 
# - Sindicato ( 5%) : R$ 
# = Salário Liquido : R$Obs.: Salário Bruto - Descontos = Salário Líquido.

ganhoPorHora = float(input('Digite o quanto você ganha:R$ '))
horasTrabalhadas = float(input('Digite quantas horas você trabalha: '))
salarioBruto = ganhoPorHora * horasTrabalhadas
impostoDeRenda = float(0.11 * salarioBruto)
inss = float(0.08 * salarioBruto)
sindicato = (0.05 * salarioBruto)
salarioLiquido = salarioBruto - (impostoDeRenda + inss + sindicato)

print('Salário Bruto:R$ ' + str(salarioBruto))
print('Imposto de renda:R$ ' + str(impostoDeRenda))
print('INSS:R$ ' + str(inss))
print('Sindicato:R$ ' + str(sindicato))
print('Salário Líquido:R$ ' + str(salarioLiquido))